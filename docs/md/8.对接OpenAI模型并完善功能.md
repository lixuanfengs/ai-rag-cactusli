...
其中 `rag` 是扩展的向量模型，可以指定使用 `OpenAi` 的向量模型或者使用 `Ollama DeepSeek`。 
...
使用 `OllamaConfig` 直接配置出`OpenAI、Ollama` 两套模型，之后根据 `spring.ai.rag.embedding` 属性的不同，实现出不同的向量库。
...
通过接口的不同 `/api/v1/ollma/`、`/api/v1/openai/` 区分出两套模型对接。
